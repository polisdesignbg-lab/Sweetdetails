...
export async function POST(request:Request){
 ...
 if(!order.product?.title||!order.contact?.email||Number(order.quantity)<10)return Response.json({error:"Минималното количество е 10 броя."},{status:400});
 ...
 const form=new URLSearchParams();form.set("mode","payment");form.set("success_url",`${origin}/?payment=success&order=${id}`);form.set("cancel_url",`${origin}/?payment=cancelled`);form.set("customer_email",String(order.contact.email));form.set("client_reference_id",id);form.set("line_items[0][price_data][currency]","bgn");form.set("line_items[0][price_data][unit_amount]",String(amount));form.set("line_items[0][price_data][product_data][name]",`${order.product.title} × ${order.quantity}`);form.set("line_items[0][quantity]","1");form.set("metadata[order_id]",id);
