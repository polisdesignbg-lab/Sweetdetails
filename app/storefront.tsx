...
export default function Storefront({initial}:Props) {
  ...
  return <main style={{"--brand":settings.primaryColor} as React.CSSProperties}>
    <div className="announcement"><span><Truck size={15}/> Минимална поръчка 10 бр.</span><span><Heart size={15} fill="currentColor"/> {settings.announcement}</span></div>
    <header className="nav shop-nav"><button className="menu left-menu" onClick={()=>setMenu(!menu)} aria-label="Меню" aria-expanded={menu}>{menu?<X/>:<Menu/>}</button><a href="#top" className="brand-logo centered-logo"><Image src="/sweet-details-logo.jpeg" alt="Sweet Details" width={96} height={96} unoptimized/></a><div className="nav-actions"><a href="#products" aria-label="Търсене"><Search/></a><a href="#products" aria-label="Кошница" className="cart-icon"><ShoppingBag/><b>0</b></a></div><nav className={menu?"open":""}><a href="#top" onClick={()=>setMenu(false)}>Начало</a><a href="#products" onClick={()=>setMenu(false)}>Бисквитки</a><a href="#how" onClick={()=>setMenu(false)}>Как се поръчва</a><a href="#about" onClick={()=>setMenu(false)}>За нас</a><a href="#contact" onClick={()=>setMenu(false)}>Контакти</a></nav></header>
    ...
    <section className="occasion-row" aria-label="Категории">{[["Кръщене",Baby],["Рожден ден",CalendarHeart],["Сватба",Gem],["Завършване",GraduationCap],["Фирмени",BriefcaseBusiness],["Други",Sparkles]].map(([c,I])=>{const Icon=I as typeof Baby;return <button key={c as string} onClick={()=>{setCategory(c as string);document.getElementById("products")?.scrollIntoView({behavior:"smooth"})}}><span><Icon/></span><strong>{c as string}</strong></button>})}</section>
    <section id="products" className="section products"><span className="section-label">Подбрани за теб</span><div className="section-head favorites-title"><h2>Най-любими <Heart/></h2><button onClick={()=>setCategory("Всички")}>Виж всички <ArrowRight/></button></div><div className="filters">{["Всички",...settings.categories].map(c=><button className={category===c?"active":""} onClick={()=>setCategory(c)} key={c}>{c}</button>)}</div><div className="grid">{filtered.map(p=><ProductCard product={p} key={p.id} onOpen={()=>setSelected(p)}/>)}</div>{!filtered.length&&<div className="empty">Скоро ще добавим предложения в тази категория.</div>}</section>
    ...
    <nav className="mobile-bottom"><a href="#top"><Home/>Начало</a><a href="#products"><Sparkles/>Категории</a><a href="#products"><ShoppingBag/>Поръчай</a><a href="#contact"><Heart/>Контакти</a></nav>{selected&&<OrderModal product={selected} onClose={()=>setSelected(null)}/>} 
  </main>
}
